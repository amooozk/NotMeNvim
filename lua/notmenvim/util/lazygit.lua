...
M.theme_path = NotMeNvim.norm(vim.fn.stdpath("cache") .. "/lazygit-theme.yml")
...
function M.open(opts)
  if vim.g.lazygit_theme ~= nil then
    NotMeNvim.deprecate("vim.g.lazygit_theme", "vim.g.lazygit_config")
  end
  ...
  if vim.g.lazygit_config then
    ...
    if not M.config_dir then
      ...
      if ok then
        ...
        vim.env.LG_CONFIG_FILE = NotMeNvim.norm(M.config_dir .. "/config.yml" .. "," .. M.theme_path)
      else
  ...
  return NotMeNvim.terminal(cmd, opts)
end
...
function M.get_color(v)
  ...
  local color = {}
  if v.fg then
    color[1] = NotMeNvim.ui.color(v.fg)
  elseif v.bg then
    color[1] = NotMeNvim.ui.color(v.bg, true)
  end
  if v.bold then
...
function M.blame_line(opts)
  ...
  local root = NotMeNvim.root.detectors.pattern(0, { ".git" })[1] or "."
...
function M.browse()
  ...
  local function open(remote)
    if remote then
      NotMeNvim.info(("Opening [%s](%s)"):format(remote.name, remote.url))
  ...
  if #remotes == 0 then
    return NotMeNvim.error("No git remotes found")
