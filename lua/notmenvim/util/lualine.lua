...
function M.cmp_source(name, icon)
  ...
  local colors = {
    ok = NotMeNvim.ui.fg("Special"),
    error = NotMeNvim.ui.fg("DiagnosticError"),
    pending = NotMeNvim.ui.fg("DiagnosticWarn"),
  }

  return {
    function()
      return icon or NotMeNvim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
    end,
    cond = function()
      return status() ~= nil
    end,
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    local root = NotMeNvim.root.get({ normalize = true })
    local cwd = NotMeNvim.root.cwd()
