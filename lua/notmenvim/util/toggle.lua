---@class notmenvim.util.toggle
local M = {}

---@class notmenvim.Toggle
---@field name string
---@field get fun():boolean
---@field set fun(state:boolean)

---@class notmenvim.Toggle.wrap: notmenvim.Toggle
---@operator call:boolean

---@param toggle notmenvim.Toggle
function M.wrap(toggle)
  return setmetatable(toggle, {
    __call = function()
      ...
      if state then
        NotMeNvim.info("Enabled " .. toggle.name, { title = toggle.name })
      else
        NotMeNvim.warn("Disabled " .. toggle.name, { title = toggle.name })
      end
      return state
    end,
  }) --[[@as notmenvim.Toggle.wrap]]
end
...
function M.map(lhs, toggle)
  ...
  NotMeNvim.safe_keymap_set("n", lhs, function()
    t()
...
function M.wk(lhs, toggle)
  if not NotMeNvim.has("which-key.nvim") then
    return
  end
  local function safe_get()
    local ok, enabled = pcall(toggle.get)
    if not ok then
      NotMeNvim.error({ "Failed to get toggle state for **" .. toggle.name .. "**:\n", enabled }, { once = true })
    end
    return enabled
  end
...
function M.format(buf)
  return M.wrap({
    ...
    get = function()
      ...
      return NotMeNvim.format.enabled()
    end,
    set = function(state)
      NotMeNvim.format.enable(state, buf)
    end,
  })
end
  ...
  set = function(state)
    if state then
      ...
      vim.api.nvim_create_autocmd("ExitPre", {
        once = true,
        group = vim.api.nvim_create_augroup("notmenvim_restore_max_exit_pre", { clear = true }),
