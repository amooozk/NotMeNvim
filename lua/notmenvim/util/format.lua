...
function M.formatexpr()
  if NotMeNvim.has("conform.nvim") then
...
function M.info(buf)
  ...
  NotMeNvim[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    NotMeNvim.warn("No formatter available", { title = "NotMeNvim" })
  end
end
...
function M.health()
  ...
  local has_extra = vim.tbl_contains(Config.spec.modules, "notmenvim.plugins.extras.lsp.none-ls")
  if has_plugin and not has_extra then
    NotMeNvim.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in NotMeNvim.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `notmenvim.plugins.extras.lsp.none-ls` extra,",
