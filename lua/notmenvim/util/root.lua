...
function M.detectors.lsp(buf)
  ...
  for _, client in pairs(NotMeNvim.lsp.get_clients({ bufnr = buf })) do
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  NotMeNvim.info(lines, { title = "NotMeNvim Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("LazyRoot", function()
    NotMeNvim.root.info()
  end, { desc = "NotMeNvim roots for the current buffer" })
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("notmenvim_root_cache", { clear = true }),
...
function M.get(opts)
  ...
  return NotMeNvim.is_win() and ret:gsub("/", "\\") or ret
end
